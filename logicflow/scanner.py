...
# Cache for _line_of: maps id(content) -> list of newline offsets
_line_of_cache: dict = {}
_line_of_refs: dict = {}  # prevent GC from reusing content ids
...
def _line_of(content, pos):
    ...
    offsets = _line_of_cache.get(id(content))
    if offsets is None:
        offsets = [i for i, c in enumerate(content) if c == '\n']
        _line_of_cache[id(content)] = offsets
        _line_of_refs[id(content)] = content  # prevent GC from reusing the id
...
_DB_PATTERNS_C = {
    lang: {
        "table_ref": [(re.compile(p, re.IGNORECASE), t) for p, t in rules["table_ref"]]  # type: ignore[attr-defined]
    }
...
class CodeScanner:
    ...
    def _scan_nextjs_app_router(self, content, rel):
        ...
        if "app/" in rel_norm and (rel_norm.endswith(("/route.ts", "/route.js"))):
